    ...
    public class Tools
    {
        public static bool StopService(string serviceName, int timeoutMilliseconds, bool dontShowMessageBoxes)
        {
            ...
            do
            {
                try
                {
                    ...
                    if (retry == false) service.Stop();
                    service.WaitForStatus(ServiceControllerStatus.Stopped, timeout);
                    if (!dontShowMessageBoxes) MessageBox.Show("Service '" + serviceName + "' was stopped successfully.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    retry = false;
                }
                catch (TimeoutException)
                {
                    var result = MessageBox.Show("A timeout occured stopping the service '" + serviceName + "'.\n" +
                                                 "Would you like to try again?", "Error", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error);
                    retry = result == DialogResult.Retry;
                }
                catch (Exception ex)
                {
                    if (!dontShowMessageBoxes) MessageBox.Show("Could not stop service '" + serviceName + "'\n" + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    retry = false;
                }

                // Maybe the service was stopped after waiting time in dialog
                service.Refresh();
                if (service.Status == ServiceControllerStatus.Stopped) retry = false;
            } while (retry);
        ...
        public static bool StartService(string serviceName, int timeoutMilliseconds, bool dontShowMessageBoxes, bool restartIfRunning)
        {
            ...
            if (restartIfRunning && service.Status == ServiceControllerStatus.Running)
                do
                {
                    ...
                    {
                        if (!dontShowMessageBoxes) MessageBox.Show("Could not stop service '" + serviceName + "' before starting.\n" + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        retry = false;
                    }
            ...
            do
            {
                ...
                {
                    var timeout = TimeSpan.FromMilliseconds(timeoutMilliseconds);
                    if (retry == false) service.Start();
                    service.WaitForStatus(ServiceControllerStatus.Running, timeout);
                    if (!dontShowMessageBoxes) MessageBox.Show("Service '" + serviceName + "' was started successfully.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    retry = false;
                }
                catch (TimeoutException)
                {
                    var result = MessageBox.Show("A timout occured starting the service '" + serviceName + "'.\n" +
                                                 "Would you like to try again?", "Error", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error);
                    retry = result == DialogResult.Retry;
                }
                catch (Exception ex)
                {
                    if (!dontShowMessageBoxes) MessageBox.Show("Could not start service '" + serviceName + "'\n" + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    retry = false;
                }

                // Maybe the service was started after waiting time in dialog
                service.Refresh();
                if (service.Status == ServiceControllerStatus.Running) retry = false;
            } while (retry);
        ...
        public static string GetS7DOSHelperServiceName()
        {
            ...
            for (var i = 0; i < services.Length; i++)
                ...
                else if (services[i].ServiceName == "s7oiehsx64") return services[i].ServiceName;
            return string.Empty;
        }
