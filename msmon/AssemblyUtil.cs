...
namespace System
{
    ...
    internal static class AssemblyUtil
    {
        ...
        {
            Type targetType = Type.GetType(fullTypeName, false, ignoreCase);
            ...
            try
            {
                ...
                string typeNamePrefix = names[0].Trim() + "`";

                Type[] matchedTypes = assembly.GetExportedTypes().Where(t => t.IsGenericType
            ...
            where TBaseInterface : class
        {
            ...
            {
                Type currentImplementType = arrType[i];
        ...
        public static T CopyPropertiesTo<T>(this T source, Predicate<PropertyInfo> predict, T target)
        {
            ...
            {
                PropertyInfo p = targetProperties[i];
        ...
        /// <summary>
        /// Get(s) assembly folder full path.
        /// </summary>
        /// <param name="assembly">Assembly instance.</param>
        /// <returns>The full path of folder where the assembly file located in.</returns>
        public static string GetFolderFullPath(this Assembly assembly)
        {
        ...
        /// <summary>
        /// Get(s) assembly name without extension.
        /// </summary>
        /// <param name="assembly">Assembly instance.</param>
        /// <returns>The assembly file name without extension.</returns>
        public static string GetFileNameWithoutExtension(this Assembly assembly)
        {
            const string ExtensionName = ".dll"; // or .exe
            string assemblyName = assembly.ManifestModule.Name;
            return assemblyName.Substring(0, assemblyName.Length - ExtensionName.Length);
        }
        ...
        /// </summary>
        /// <returns>The assembly file name without extension.</returns>
        public static void MakeLog4netConfigTakeEffect(this object @object, string logDirectory)
        {
            const string LOG4NET_CONFIG_RESOURCE_NAME = "log4net.config";
            string manifestResourceName = typeof(AssemblyUtil).Assembly.GetManifestResourceNames().
                                                  FirstOrDefault(x => x.ToUpperInvariant().
                                                                                 Contains(LOG4NET_CONFIG_RESOURCE_NAME.ToUpperInvariant()));

            if (!string.IsNullOrEmpty(manifestResourceName))
            {
                using (Stream stream = typeof(AssemblyUtil).Assembly.GetManifestResourceStream(manifestResourceName))
                {
                    byte[] buffer = new byte[stream.Length];
                    stream.Read(buffer, 0, buffer.Length);
                    string content = Encoding.UTF8.GetString(buffer);
                    const string origin = "file value=\"Logs\\";
                    if (string.IsNullOrEmpty(logDirectory))
                    {
                        content = content.Replace(origin,
                        string.Format("file value=\"{0}\\", Path.Combine(typeof(AssemblyUtil).Assembly.GetFolderFullPath(), "Logs")));
                    }
                    else
                    {
                        content = content.Replace(origin, string.Format("file value=\"{0}\\", logDirectory));
                    }
            ...
            {
                throw new FileNotFoundException("Missing log configuration file: {log4net.config}.");
            }
        }
